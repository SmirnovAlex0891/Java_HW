...
public class JavaStreamTask1 {
    public static void main(String[] args) {
        ...
        matches.stream()
                .filter(e -> e.getDate().get(Calendar.DATE) < 15)
                .forEach(System.out::println);

        // посчитать общее количество матчей в выборке

        System.out.println("Oбщее количество матчей в выборке: "+ matches.stream()
                .count());
        ...
        matches.stream()
                .filter(e -> e.getStadium().equals("WWK Arena"))
                .sorted((e1, e2) -> Math.abs(e2.getResult1()- e2.getResult2()) - Math.abs(e1.getResult1() - e1.getResult2()))
                .limit(3)
                .forEach(System.out::println);

        // посчитать сколько матчей содержат упоминания о стадионе
        System.out.println(matches.stream()
                .filter(e -> !e.getStadium().equals("NULL"))
                .count());
        ...
        for (Match m : matches) {
            if (map1.containsKey(m.getTeam2())) {
                map1.put(m.getTeam2(), map1.get(m.getTeam2()) + 1);
            } else {
                map1.put(m.getTeam2(), 1);
            }
        }
        ...
        Map<String, Long> map = matches.stream()
        ...
        matches.stream()
                .collect(Collectors.groupingBy(Match::getDate, Collectors.counting()))
                .entrySet().stream()
                .max(Map.Entry.comparingByValue())
                .ifPresent(e -> System.out.println(e.getKey().get(Calendar.YEAR) + "-" + e.getKey().get(Calendar.MONTH) + "-" + e.getKey().get(Calendar.DATE) + " : " + e.getValue()));
